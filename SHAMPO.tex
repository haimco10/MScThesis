...
When we examine the extream cases  of $b$ we see that  for $b\rightarrow0$
...
shows an example of the distribution for the case of two tasks ($K=2$) at an arbitrary step $t^*$. 
For visualization purpose, we fix 
$\abs{\hat{p}_{2,t^*}}=0.5$ and see how the probability of task $1$ to be chosen, 
changed, for varying $b$ and $\abs{\hat{p}_{1,t^*}}$ values. Three different vertical areas  can be easily seen 
in the graph. The upper (green) area, where $b>>\max{\braces{\hat{p}_{1,{t^*}},\hat{p}_{2,t{^*}}}$,
 ...
 which represents the model exploration over the tasks.
