...
\caption{Number of queries vs. number of errors, USPS one-vs-rest, all tasks}
\label{fig:qr_er}
\end{centering}
\end{figure}
...
\eqref{fig:qr_step}. The FO algorithm run (\figref{fig:qr_step_FO}) ends with $100-1,400$ mistakes for each task and alternate 
...
another time intervals repeatedly. This probably shows that this algorithm is 
less stable than the others. The SO algorithm run (\figref{fig:qr_step_SO})  ends with $50-2,000$ mistakes for each 
task, yet it focuses mainly on two tasks while the rest didn't have more than $900$ 
prediction mistakes. The SOAG algorithm (\figref{fig:qr_step_SOAG}) uses the exploration-exploitation remarkably. It 
focuses more on (probably) harder tasks (expitation), since we see that it ends with $550-950$ mistakes for each 
...
behavior of the algorithm it that when the number of mistakes goes up (harder 
task), the number of queries will rise as well. In fact the relation between 
margin and high number of mistakes may not strictly holds for each task and 
...
in the other hand, shows the desired property of SHAMPO algorithm along all of the run time. 
...
were done on the same task, which is a bad behavior. Saying that, this is not always a bad when this 
...
number of queries for the tasks, which in turn increase this ratio. In this case, we expect an exploration 
which cause all the tasks to share 
...
means that the algorithm makes a lot of errors on a certain task, but does not 
issue enough queries on that task in order to improve the model and decrease the number of 
errors. We see in the FO (\eqref{fig:qr_er_frac_FO}) and even stronger in the SO 
(\eqref{fig:qr_er_frac_SO}) algorithm that there is difference between the 
task with the highest value and the task with the lowest value (1-7 for FO and 0.5-10 for 
SO).

In this experiment we saw that even that we want the algorithm to concentrate on 
...
idea. We also noticed that the Second Order Aggressive algorithm gives high 
performance, not only in sense of prediction mistakes, but also in the 
explration exploitation manner.
...
\begin{figure}[!htb]
\begin{centering}
\subfigure[MNIST one-vs-one]{\includegraphics[width=0.37\textwidth]{figs/queried-m1.eps}\label{fig:queried_m1}}\hfill
