...
$\cQ \sim\LL\paren{\vmuq,\sigmaq}$.
 % Let $\paren{ \vxii , \yii }$ be an example that is classified correctly by the majority of weights according to $\cQ$, that is $\yii \paren{ \vmuq \cdot \vxii }\geq 0 $.
The expected zero-one loss  of an example $(\vx,y)$, with all elements of $\vx$
differ from each other, is given by,
\vspace{-0.35cm}
\begin{align*} 
%\prp{\vomega\sim\cQ}{ y \paren{ \vomega \cdot \vx }\leq 0  } &=
...
\end{align*}
\vspace{-0.07cm}
where the first equality  %states equivalent quantities and 
defines 
$\ellcdf$ and,
\vspace{-0.3cm}
\begin{align*}
...
\end{align*} 
\vspace{-0.5cm}
%\kc{need for a plot of $\alpha_k(\vx)$ and $\mcal{E}(\vx, y, \vmu, \vsigma) %$ for some $\vx$}
%\asaf{I made plots for $\mcal{E}$, not sure what is your meaning by a plot of $\alpha_k(\vx)$ }
%\label{theoremErrorConsts}
%\end{lemma}
\end{block}
\end{frame}
