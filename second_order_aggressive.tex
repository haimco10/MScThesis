\section{Second Order Adaptive SHAMPO}
...
All the algorithms that were shown so far, exploited the uncertainty of the algorithm prediction using 
the low margin rule. Another approach of estimating the certainty of prediction could be 
comparing the input example with all the previous tagged examples. If the 
...
another quantity that is the projection of the vector $\vxiit$ on the inverse of the covariance-like matrix $A_{i,t-1}$ 
\[
r_{i,t} = \vxiit^TA_{i,t-1}^{-1}\vxiit
\]
...
to low uncertainty, since the algorithm already received labeled that are close 
